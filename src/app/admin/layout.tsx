...
import { 
  ...
  Package, 
  Handshake, 
  History as HistoryIcon, // 🔥 ALIAS: History → HistoryIcon
  Users, 
  Building2,
  LogOut,
  User,
...
const navItems: NavItem[] = [
  ...
  { name: 'Sorting Sessions', href: '/admin/sorting', icon: <Package className="w-4 h-4" />, category: 'B2C' },
  { name: 'Handover Manifest', href: '/admin/manifest', icon: <FileText className="w-4 h-4" />, category: 'B2C' },
  
  // 🔥 INSTANT Menu (Putaway & Pickup)
  { name: 'Putaway', href: '/putaway', icon: <Box className="w-4 h-4" />, category: 'INSTANT' },
  { name: 'Pickup', href: '/pickup', icon: <Search className="w-4 h-4" />, category: 'INSTANT' },
  
  // B2B Menu (Coming Soon)
  { name: 'B2B Management', href: '#', icon: <Building2 className="w-4 h-4" />, category: 'B2B', disabled: true },
  
  // 🔥 Admin Menu - PAKAI HistoryIcon
  { name: 'User Management', href: '/admin/users', icon: <Users className="w-4 h-4" />, category: 'ADMIN' },
  { name: 'History Logs', href: '/admin/history', icon: <HistoryIcon className="w-4 h-4" />, category: 'ADMIN' },
];
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              {navItems.filter(item => item.category === 'INSTANT').map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`
